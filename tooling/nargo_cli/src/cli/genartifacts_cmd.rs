...
use super::{
    ...
    fs::{inputs::read_inputs_from_file, load_hex_data, create_named_dir, write_to_file}
};
...
fn generate_intermediary_artifacts(    
    ...
    let (proof_as_fields, vk_hash, vk_as_fields, contract_code) = backend.get_intermediate_proof_artifacts( &compiled_program.circuit, proof.as_slice(), public_inputs)?;
